...
export function StoreProvider(props: ComponentProps<IStoreData>) {
  ...
  const [audioStartTime, setAudioStartTime] = createSignal(
    props.audioStartTime,
  );
  const [audioCurrentTime, setAudioCurrentTime] = createSignal(
    props.audioCurrentTime,
  );
  const [audioCurrentTimeNonCapture, setAudioCurrentTimeNonCapture] =
    createSignal(props.audioCurrentTimeNonCapture);
  const [audioLoaded, setAudioLoaded] = createSignal(props.audioLoaded);
  const [audioPlayerState, setAudioPlayerState] = createSignal(
    props.audioPlayerState,
  );
  const [audioTrackerState, setAudioTrackerState] = createSignal(
    props.audioTrackerState,
  );
  const [pressedKey, setPressedKey] = createSignal(props.pressedKey);
  const [pageSurahAudioTimeStamps, setPageSurahAudioTimeStamps] = createSignal<
    Array<IReciterTimeStamp>
  >(props.pageSurahAudioTimeStamps);
  const [ayahInCurrentPageSurah, setAyahInCurrentPageSurah] = createSignal<
    Array<IAyahBase>
  >(props.ayahInCurrentPageSurah);
  const [captureIndex, setCaptureIndex] = createSignal(props.captureIndex);
  const [saveClickCounter, setSaveClickCounter] = createSignal(
    props.saveClickCounter,
  );
